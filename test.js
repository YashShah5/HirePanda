...
def validate_auth(token, org_name, base_url=None, label=""):
    if not token or len(token.strip()) < 10:
        raise ValueError(
            f"GitHub token is missing or malformed for {label}"
        )
...
def verify_tags():
    try:
        ...
        source_gh, _ = validate_auth(
            SOURCE_TOKEN, SOURCE_ORG, SOURCE_BASE_URL, label="source"
        )
        destination_gh, _ = validate_auth(
        ...
        destination_repo = destination_gh.get_repo(
            f"{DESTINATION_ORG}/{DESTINATION_REPO}"
        )
